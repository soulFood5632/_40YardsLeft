...
extension Round: Equatable {
  static func == (rhs: Round, lhs: Round) -> Bool {
    return rhs.id == lhs.id && rhs.course == lhs.course && rhs.holes == lhs.holes
      && rhs.tee == lhs.tee
  }
}
...
extension Round {
  ...
  static let emptyRoundExample1 = try! Round(
    course: .example1, tee: Course.example1.listOfTees[0], date: .now, roundType: .casual)
  /// An example round which contains the score 4 on every hole.
  static let completeRoundExample1 = {
    var round = try! Round(
      course: .example1, tee: Course.example1.listOfTees[0], roundType: .competative)

    let sampleShotList: [Shot] = [
      .init(
        type: .drive, startPosition: Position(lie: .tee, yardage: .yards(430)),
        endPosition: Position(lie: .fairway, yardage: .yards(196))),
      Shot(
        type: .approach, startPosition: .init(lie: .fairway, yardage: .yards(196)),
        endPosition: .init(lie: .green, yardage: .feet(35))),
      Shot(
        type: .putt, startPosition: .init(lie: .green, yardage: .feet(35)),
        endPosition: .init(lie: .green, yardage: .feet(2))),
      Shot(
        type: .putt, startPosition: .init(lie: .green, yardage: .feet(2)),
        endPosition: .holed),
    ]
  ...
  func getShots() -> [Shot] {
    do {
      ...
      preconditionFailure(
        "The method gets shots was called before all of the holes were finished")
    }
  ...
  var differential: Double {
    ...
    return (Double(self.roundScore) - self.tee.rating) * Double(self.tee.slope)
      / Double(Self.SLOPE_CONSTANT)
  }
  ...
  func strokesGainedApproach() -> Double {
    do {
      ...
      preconditionFailure(
        "all holes should be complete prior to the calling of this method.")
    }
  }
