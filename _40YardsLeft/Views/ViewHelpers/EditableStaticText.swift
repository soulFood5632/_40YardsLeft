...
struct EditableStaticText: View {
  ...
  var body: some View {
    HStack {
      if isClicked {
        let mustNotBeEmptyAndEqual = TextValiditity.combine(conditions: [
          TextValiditity.betweenSizes(range: 1..<15),
          TextValiditity.mustNotBeEqualTo(originalText),
        ])

        TextFieldWithValiditity(
          condition: mustNotBeEmptyAndEqual, text: $text, prompt: "",
          isSecureField: false, isValid: self.$isTextValid
        )
        .padding(.trailing)
...
struct EditableStaticText_Previews: PreviewProvider {
  ...
  static var previews: some View {
    EditableStaticText(
      text: self.$text, isConfirmed: self.$isConfirmed, originalText: self.text)
  }
}
