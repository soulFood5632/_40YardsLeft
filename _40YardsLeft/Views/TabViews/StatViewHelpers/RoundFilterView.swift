            ...
            GroupBox {
                ...
                HStack {
                    ForEach(RoundType.allCases.sorted { $0.rawValue < $1.rawValue }) { type in
                        ...
                        Label(type.rawValue, systemImage: roundType.contains(type) ? "checkmark.circle" : "circle")
...
struct RoundFilterView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        RoundFilterView(startDate: self.$startDate, endDate: self.$endDate, roundType: self.$roundType, ratingBound: $ratingBound)
    }
}
