...
struct ChippingAnalysisView: View {
  ...
  var strokesGained: (Double, Int) {
    return (
      ...
      shots.filter(ShotFilters.allShortGame).count
    )
  }

  var inside8Percentage: (Double?, Int) {
    return (
      shots.percentageInsideProx(
        maxDistance: .feet(8), range: .zero..<Distance.yards(100), lie: Lie.allCases,
        shotType: .chip_pitch), shots.filter { $0.type == .chip_pitch }.count
    )
  }

  var proximity: (Distance?, Int) {
    return (
      shots.approachProximityFrom(
        range: .zero..<Distance.MAX_DISTANCE, lie: Lie.allCases, shotType: .chip_pitch),
      shots.filter { $0.type == .chip_pitch }.count
    )
  }
  ...
  func strokesGainedWithFilters(range: Range<Distance>, lies: [Lie]) -> (Double, Int) {
    ...
    let strokesGained = shots.strokesGained { shot in
      return range.contains(shot.startPosition.yardage)
  ...
  func proximityInRange(range: Range<Distance>) -> (Distance?, Int) {
    (
      shots.approachProximityFrom(range: range, lie: Lie.allCases, shotType: .chip_pitch),
      shots.filter { $0.type == .chip_pitch && range.contains($0.startPosition.yardage) }
        .count
    )
  }
  ...
  func proximityInRangeWithLies(range: Range<Distance>, lies: [Lie]) -> (Distance?, Int) {
    (
      shots.approachProximityFrom(range: range, lie: lies, shotType: .chip_pitch),
      shots.filter {
        $0.type == .chip_pitch && range.contains($0.startPosition.yardage)
          && lies.contains($0.startPosition.lie)
      }.count
    )
  }
  ...
  func inside8feetRate(range: Range<Distance>, lies: [Lie]) -> (Double?, Int) {
    (
      shots.percentageInsideProx(
        maxDistance: .feet(8), range: range, lie: lies, shotType: .chip_pitch),
      shots.filter {
        $0.type == .chip_pitch && range.contains($0.startPosition.yardage)
          && lies.contains($0.startPosition.lie)
      }.count
      ...
      GroupBox {
        Grid(horizontalSpacing: 20) {
          GridRow {
            ...
            VStack {
              let strokesGainedData = self.strokesGainedWithFilters(
                range: self.distanceBounds, lies: self.lies)
              ...
              if strokesGainedData.1 == 0 {
                ...
                Text(
                  "\(strokesGainedData.0 / Double(strokesGainedData.1), specifier: "%.2f") per shot"
                )
                .font(.caption)
              }
            }
          ...
          GridRow {
            ...
            let proxData = self.proximityInRangeWithLies(
              range: self.distanceBounds, lies: self.lies)
            VStack {
          ...
          GridRow {
            ...
            let successData = self.inside8feetRate(
              range: self.distanceBounds, lies: self.lies)
            VStack {
