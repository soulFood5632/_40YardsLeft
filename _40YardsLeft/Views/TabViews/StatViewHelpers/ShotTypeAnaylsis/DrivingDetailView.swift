...
struct DrivingDetailView: View {
    ...
    var strokesGained: (Double, Int) {
        return (shots.strokesGained(ShotFilters.allTeeShots), shots.filter { $0.type == .drive }.count)
    }
    
    var lostBallPercentage: (Double?, Int) {
        return (shots.percentageLostBall(shotType: .drive), shots.filter { $0.type == .drive}.count)
    }
    
    var treePercentage: (Double?, Int) {
        return (shots.percentageEndingIn(lies: [.recovery], shotType: .drive), shots.filter { $0.type == .drive }.count)
    }
    
    var drivingDistance: (Distance?, Int) {
        return (shots.map { $0.advancementYardage }.average(), shots.filter { $0.type == .drive }.count)
    }
            ...
            GroupBox {
                Grid (horizontalSpacing: 10) {
                    ...
                    GridRow {
                        ...
                        let combinedPercentages = (self.treePercentage.0 ?? 0) + (self.lostBallPercentage.0 ?? 0)
                        VStack {
