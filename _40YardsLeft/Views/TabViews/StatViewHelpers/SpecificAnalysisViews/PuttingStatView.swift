...
struct PuttingStatView: View {
    ...
    @State private var distanceBounds: Range<Distance> = Distance(feet: 10)..<Distance(feet: 50)
    ...
    var puttingRanges: [Range<Distance>] { PuttingDetailView.getSplitRegoins(at: [3, 5, 7, 10, 15, 20, 30, 60])
    }
    
    var strokesGained: [DisplayStat<Double, Int>] {
        let perShot = rounds.strokesGained(for: .putt).0 / Double(rounds.strokesGained(for: .putt).1)
        
        let perRound = rounds.strokesGained(for: .putt ).0 / Double(rounds.strokesGained(for: .putt).2)
    ...
    var makeStats: [DisplayStat<Double, Int>] {
        ...
        for range in self.puttingRanges {
            ...
            if range.upperBound == Distance.MAX_DISTANCE {
                ...
                headerString = String(format: "%.0f", range.lowerBound.feet) + " - " + String(format: "%.0f", range.upperBound.feet)
            }
            
            
            
            statList.append(DisplayStat(name: headerString,
                                        value: (shots.makePercentageFrom(range: range) ?? 0) * 100,
                                        numOfSamples: shots.filter(ShotFilters.allPutts).filter { range.contains($0.startPosition.yardage) }.count,
                                        formatter: "%.1f",
                                        isPercent: true))
        }
