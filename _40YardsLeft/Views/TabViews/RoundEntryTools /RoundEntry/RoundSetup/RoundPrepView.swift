...
struct RoundSetupBuffer {
  ...
  func createRound(course: Course) -> Round {
    ...
    return try! Round(
      course: course, tee: self.tee!, date: self.date, roundType: roundType)
  }
  ...
  var body: some View {
    ...
    VStack {
      GroupBox {
        List {
          ...
          ForEach(course.listOfTees) { tee in
            ...
            HStack {
              ...
              Divider()
              Text(
                "\(tee.rating, format: .number), \(tee.slope), \(tee.yardage.yards, format: .number), \(tee.par)"
              )
              .fontWeight(.light)
        ...
        Label(
          "Choose Tee", systemImage: "dots.and.line.vertical.and.cursorarrow.rectangle")
      }
      ...
      GroupBox {
        Form {
          ...
          HStack {
            ...
            PickerAndLabel(
              pickedElement: self.$buffer.roundType, choices: RoundType.allCases,
              title: "Round Type")
    ...
    .sheet(isPresented: self.$isCreateNewTee) {
      CourseHoleByHole(
        course: self.$course, showView: self.$isCreateNewTee,
        teeTemplate: self.course.listOfTees.first
      )
      .padding()
    }
    .onDisappear {
    ...
    .toolbar {
      ...
      ToolbarItem(placement: .navigationBarTrailing) {
        ...
        .confirmationDialog(
          "Delete Round", isPresented: self.$isCancelledRound,
          actions: {
...
struct RoundPrepView_Previews: PreviewProvider {
  ...
  static var previews: some View {
    NavigationStack {
      RoundPrepView(
        course: self.$course, golfer: self.$golfer, round: $round, path: self.$path)
    }
  }
}
