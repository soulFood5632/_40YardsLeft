...
extension Array where Element == Shot {
  ...
  func approachProximityFrom(range: Range<Distance>, lie: [Lie], shotType: ShotType)
    -> Distance?
  {
    self.filter { $0.type == shotType }
      .filter {
        lie.contains($0.startPosition.lie) && range.contains($0.startPosition.yardage)
      }
      .map { $0.endPosition.yardage }
      .average()
  }
  ...
  func greenPercentageFrom(range: Range<Distance>, lie: [Lie], shotType: ShotType)
    -> Double?
  {
  ...
  func percentageInsideProx(
    maxDistance: Distance, range: Range<Distance>, lie: [Lie], shotType: ShotType
  ) -> Double? {
    let value =
      self
  ...
  func strokesGained(_ filter: @escaping (Shot) -> Bool) -> Double {
    ...
    self.filter(filter)
      .forEach { shot in
        if shot.type == .putt {
          print(
            "\(shot.startPosition.yardage.feet)  -> \(shot.endPosition.yardage.feet), SG: \(shot.strokesGained)"
          )
        }
      }
...
extension Array where Element == Round {
  ...
  func strokesGained(for type: ShotType) -> (Double, Int, Int) {
    do {
      ...
      return (
        shots.strokesGained { $0.type == type }, shots.filter { $0.type == type }.count,
        self.count
      )

    } catch {
      preconditionFailure(
        "This function should not be called when the round is not completed")
      return (0, 0, 0)
    }
  }
  ...
  func saves(for lies: [Lie]) -> (Int, Int) {
    do {
      ...
      holeShots.forEach { shots in
        ...
        while index < shots.count {
          if shots[index].type == .chip_pitch
            && lies.contains(shots[index].startPosition.lie)
          {
            // first we see if there was a hole out. If there was then that means the up and down was succseseful.
            if index + 1 == shots.count {
              sandSaveList.append(true)
