...
struct Shot: Codable, Hashable, Identifiable {
  ...
  init(
    type: ShotType, startPosition: Position, endPosition: Position, includesPenalty: Bool
  ) {
    self.type = type
    ...
    self.id = UUID()
    do {
      self.strokesGained = try Self.getStrokesGained(
        start: startPosition, end: endPosition, includesPenalty: includesPenalty)
    } catch {
  ...
  init(type: ShotType, startPosition: Position, endPosition: Position) {
    ...
    do {
      self.strokesGained = try Self.getStrokesGained(
        start: startPosition, end: endPosition, includesPenalty: false)
    } catch {
      self.strokesGained = nil
    }
  }
  ...
  private static func getStrokesGained(
    start: Position, end: Position, includesPenalty: Bool
  ) throws -> Double {
    if includesPenalty {
      ...
      if randomValue < 20 {
        shotList.append(
          Shot(
            type: .drive,
            startPosition: .init(
              lie: .tee, yardage: .init(yards: Int.random(in: 350..<450))),
            endPosition: .init(
              lie: [.fairway, .rough].randomElement()!,
              yardage: .init(yards: Int.random(in: 100..<175)))))
        ...
        shotList.append(
          Shot(
            ...
            startPosition: .init(
              lie: [.fairway, .rough].randomElement()!,
              yardage: .init(yards: Int.random(in: 50..<250))),
            endPosition: .init(
              lie: [.green, .rough, .bunker].randomElement()!,
              yardage: .init(feet: Int.random(in: 1..<120)))))
      } else if randomValue < 70 {
        shotList.append(
          Shot(
            type: .chip_pitch,
            startPosition: .init(
              lie: [.fairway, .rough, .bunker].randomElement()!,
              yardage: .init(yards: Int.random(in: 10..<50))),
            endPosition: .init(lie: .green, yardage: .init(feet: Int.random(in: 1..<50))))
        )
        ...
        shotList.append(
          Shot(
            ...
            startPosition: .init(
              lie: [.fairway, .rough].randomElement()!,
              yardage: .init(yards: Int.random(in: 50..<400))),
            endPosition: .init(
              lie: [.fairway, .rough].randomElement()!,
              yardage: .init(yards: Int.random(in: 25..<200)))))
      } else {
        shotList.append(
          Shot(
            type: .putt,
            startPosition: .init(
              lie: .green, yardage: .init(feet: Int.random(in: 1..<90))),
            endPosition: [
              .holed, .init(lie: .green, yardage: .init(feet: Int.random(in: 1..<9))),
            ].randomElement()!))
      }
...
extension Shot {
  ...
  func toString() -> String {
    if self.includesPenalty {
      return self.startPosition.toString() + " -> PENALTY -> "
        + self.endPosition.toString()
    }
