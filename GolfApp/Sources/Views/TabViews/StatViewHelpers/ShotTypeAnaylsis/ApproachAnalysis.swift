...
struct ApproachAnalysis: View {
  ...
  var strokesGained: (Double, Int) {
    return (
      ...
      shots.filter { $0.type == .approach }.count
    )
  }

  var successRate: (Double?, Int) {
    return (
      shots.greenPercentageFrom(
        range: .zero..<Distance.MAX_DISTANCE, lie: Lie.allCases, shotType: .approach),
      shots.filter { $0.type == .approach }.count
    )
  }
  ...
  func strokesGainedWithFilters(range: Range<Distance>, lies: [Lie]) -> (Double, Int) {
    ...
    let strokesGained = shots.strokesGained { shot in
      return range.contains(shot.startPosition.yardage)
        && lies.contains(shot.startPosition.lie) && shot.type == .approach
  ...
  func proximityInRange(range: Range<Distance>) -> (Distance?, Int) {
    (
      shots.approachProximityFrom(range: range, lie: Lie.allCases, shotType: .approach),
      shots.filter { $0.type == .approach && range.contains($0.startPosition.yardage) }
        .count
    )
  }
  ...
  func proximityInRangeWithLies(range: Range<Distance>, lies: [Lie]) -> (Distance?, Int) {
    (
      shots.approachProximityFrom(range: range, lie: lies, shotType: .approach),
      shots.filter {
        $0.type == .approach && range.contains($0.startPosition.yardage)
  ...
  func successInRangeWithLies(range: Range<Distance>, lies: [Lie]) -> (Double?, Int) {
    (
      shots.greenPercentageFrom(range: range, lie: lies, shotType: .approach),
      shots.filter {
        $0.type == .approach && range.contains($0.startPosition.yardage)
          && lies.contains($0.startPosition.lie)
      }.count
      ...
      GroupBox {
        Grid(horizontalSpacing: 20) {
          ...
          GridRow {
            ...
            let successData = self.successInRangeWithLies(
              range: self.distanceBounds, lies: self.lies)
            VStack {
