  ...
  var body: some View {
    ...
    Group {
      if !isFinal {
        ...
        }  // empty label becuase it does not matter.
        .pickerStyle(.menu)
        .onTapGesture(perform: {
          textFocus = false
        })
        ...
        VStack {
          let isOnGreen = shot.position.lie == .green
          Text(
            "\(isOnGreen ? shot.position.yardage.feet : shot.position.yardage.yards, format: .number)"
          )
...
struct ShotElement_Previews: View {
  @State private var shot = ShotIntermediate(
    position: Position(lie: .fairway, yardage: Distance(yards: 150)), declaration: .drive)
