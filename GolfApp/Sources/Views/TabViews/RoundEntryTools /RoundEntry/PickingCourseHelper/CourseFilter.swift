...
struct CourseFilter: View {
  ...
  var body: some View {
    VStack {
      Grid(alignment: .leading) {
        GridRow {
          ...
          PickerAndLabel(
            pickedElement: self.$filter.country, choices: Country.allCases,
            title: "Country")
        ...
        GridRow {
          ...
          PickerAndLabel(
            pickedElement: self.$filter.province,
            choices: self.filter.country.getPairedProvinces(), title: "Province/State")
    ...
    .sheet(
      isPresented: self.$isQueryReady,
      onDismiss: {

      },
      content: {
        CourseSearcher(
          overallQuery: self.courseQuery, chosenCourse: self.$chosenCourse,
          showView: self.$isQueryReady)
      }
    )
...
extension CourseFilter {
  ...
  private func searchDatabase() {
    ...
    Task {
      do {
        self.courseQuery = try await DatabaseCommunicator.getCoursesFromFilter(filter)
          .sorted(by: { no1, no2 in
            no1.name > no2.name
          })
