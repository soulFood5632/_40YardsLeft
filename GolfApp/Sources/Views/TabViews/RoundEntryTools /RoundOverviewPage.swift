...
struct RoundOverviewPage_Previews: PreviewProvider {
  ...
  static var previews: some View {
    NavigationStack {
      RoundOverviewPage(
        golfer: $golfer, path: self.$path, showView: self.$showView, round: round,
        isStat: true)
    }
...
extension Round {
  ...
  private func countOfHolesThatMatchScoreToPar(range: ClosedRange<Int>) -> Int {
    return self.holes.filter { $0.isComplete }.filter { range.contains($0.scoreToPar) }
      .count
  }
  ...
  func fairways() -> (Int, Int) {
    return self.holes.map { $0.fairway }.filter { $0 != nil }.reduce((0, 0)) {
      partialResult, fairway in
