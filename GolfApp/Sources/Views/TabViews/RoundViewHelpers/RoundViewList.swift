...
struct RoundViewList: View {
  ...
  var body: some View {
    ...
    VStack {
      ...
      List(golfer.roundsSortedByDate) { round in
        ...
        HStack {
          ...
          Menu {
            ...
            .confirmationDialog(
              "Cancel Round", isPresented: self.$isDeleteRoundBool,
              actions: {
                Button(role: .destructive) {
      ...
      .onChange(
        of: self.highlightedRound,
        perform: { _ in
          showSpecificRoundView = true
        }
      )
      .sheet(
        isPresented: self.$showSpecificRoundView,
        content: {
          RoundOverviewPage(
            golfer: self.$golfer, path: self.$path, showView: self.$showSpecificRoundView,
            round: self.highlightedRound, isStat: true)
